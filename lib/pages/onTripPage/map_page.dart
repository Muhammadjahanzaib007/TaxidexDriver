...
import 'package:taxidex_driver/functions/geohash.dart';
import 'package:taxidex_driver/pages/chatPage/chat_page.dart';
import 'package:taxidex_driver/pages/onTripPage/invoice.dart';
import 'package:taxidex_driver/pages/loadingPage/loading.dart';
import 'package:taxidex_driver/pages/login/login.dart';
import 'package:taxidex_driver/pages/navDrawer/nav_drawer.dart';
...
//getting permission and current location
      ...
      if (serviceEnabled == true) {
        ...
        if (center == null) {
          ...
          if (locs != null) {
            ...
            center = LatLng(double.parse(loc.latitude.toString()), double.parse(loc.longitude.toString()));
            heading = loc.heading;
          }
      ...
      if (permission == geolocator.LocationPermission.denied ||
        ...
        if (permission != geolocator.LocationPermission.deniedForever &&
          ...
          if (platform == TargetPlatform.android) {
            ...
            [perm.Permission.location, perm.Permission.locationAlways].request();
          }
        }
      }
    }
  ...
  getLatLngBounds() {
    LatLngBounds bound;
    if (driverReq['pick_lat'] > driverReq['drop_lat'] && driverReq['pick_lng'] > driverReq['drop_lng']) {
      bound = LatLngBounds(
  ...
  Widget build(BuildContext context) {
    ...
    capturePng(GlobalKey iconKeys) async {
      ...
      try {
        RenderRepaintBoundary boundary = iconKeys.currentContext!.findRenderObject() as RenderRepaintBoundary;
    ...
    addDropMarker() async {
      ...
      if (testIcon != null) {
        ...
        if (polyline.where((element) => element.polylineId == const PolylineId('1')).isEmpty) {
          getPolylines();
        }
      }
          ...
          builder: (context, value, child) {
            if (myMarkers.where((element) => element.markerId == const MarkerId('1')).isNotEmpty &&
              ...
              var dist = calculateDistance(
                  myMarkers.firstWhere((element) => element.markerId == const MarkerId('1')).position.latitude,
                  myMarkers.firstWhere((element) => element.markerId == const MarkerId('1')).position.longitude,
                  center.latitude,
                  center.longitude);
            ...
            if (driverReq.isNotEmpty) {
              ...
              if (driverReq['is_trip_start'] != 1) {
                if (myMarkers.where((element) => element.markerId == const MarkerId('2')).isEmpty) {
                  Future.delayed(const Duration(seconds: 2), () {
                    addMarker();
                  });
                }
                ...
                if (_pickAnimateDone != true) {
                  ...
                  Future.delayed(const Duration(seconds: 2), () {
                    _controller?.animateCamera(
                        CameraUpdate.newLatLngZoom(LatLng(driverReq['pick_lat'], driverReq['pick_lng']), 11.0));
                  });
                }
              } else if (driverReq['is_trip_start'] == 1 && driverReq['is_completed'] == 0) {
              ...
              } else if (driverReq['is_completed'] == 1 && driverReq['requestBill'] != null) {
                WidgetsBinding.instance.addPostFrameCallback((_) {
                  Navigator.pushAndRemoveUntil(
                      context, MaterialPageRoute(builder: (context) => const Invoice()), (route) => false);
                });
                _pickAnimateDone = false;
                myMarkers.removeWhere((element) => element.markerId == const MarkerId('2'));
                myMarkers.removeWhere((element) => element.markerId == const MarkerId('3'));
                polyline.removeWhere((element) => element.polylineId == const PolylineId('1'));
              }
            } else {
              mapPadding = 0;
              if (myMarkers.where((element) => element.markerId == const MarkerId('2')).isNotEmpty) {
                myMarkers.removeWhere((element) => element.markerId == const MarkerId('2'));
                if (userReject == true) {
                  reqCancel();
                }
                          ...
                          if (element['l'] != null && element['is_deleted'] != 1) {
                            if (userDetails['role'] == 'owner') {
                              if (userDetails['role'] == 'owner' &&
                                  ...
                                  filtericon == 0) {
                                if (myMarkers
                                    .where((e) => e.markerId.toString().contains('car${element['id']}'))
                                    .isEmpty) {
                                  myMarkers.add(Marker(
                                    markerId: MarkerId('car${element['id']}'),
                                    rotation: double.parse(element['bearing'].toString()),
                                    position: LatLng(element['l'][0], element['l'][1]),
                                    infoWindow: InfoWindow(title: element['vehicle_number'], snippet: element['name']),
                                    anchor: const Offset(0.5, 0.5),
                                    ...
                                    myMarkers
                                            .lastWhere((e) => e.markerId.toString().contains('car${element['id']}'))
                                            .icon ==
                                        offlineicon) {
                                  myMarkers.removeWhere((e) => e.markerId.toString().contains('car${element['id']}'));
                                  myMarkers.add(Marker(
                                    markerId: MarkerId('car${element['id']}'),
                                    rotation: double.parse(element['bearing'].toString()),
                                    position: LatLng(element['l'][0], element['l'][1]),
                              ...
                              } else if (filtericon == 1 && userDetails['role'] == 'owner' && onlineicon != null) {
                                if (element['l'] != null) {
                                  if (element['is_active'] == 0 && offlineicon != null) {
                                    if (myMarkers
                                        ...
                                        .isEmpty) {
                                      myMarkers.add(Marker(
                                        markerId: MarkerId('carid${element['id']}idoffline'),
                                        rotation: double.parse(element['bearing'].toString()),
                                        position: LatLng(element['l'][0], element['l'][1]),
                                        anchor: const Offset(0.5, 0.5),
                                        icon: offlineicon,
                                      ...
                                      if (myMarkers
                                                  .lastWhere(
                                                      (e) => e.markerId.toString().contains('car${element['id']}'))
                                                  .position
                                                  .latitude !=
                                              element['l'][0] ||
                                          myMarkers
                                    ...
                                    if (myMarkers
                                        ...
                                        .isNotEmpty) {
                                      myMarkers
                                          .removeWhere((e) => e.markerId.toString().contains('car${element['id']}'));
                                    }
                                  }
                                }
                              ...
                              } else if (filtericon == 3 && userDetails['role'] == 'owner' && onlineicon != null) {
                                if (element['is_available'] == true && element['is_active'] == 1) {
                                  if (myMarkers
                                      .where((e) => e.markerId.toString().contains('car${element['id']}'))
                                      .isEmpty) {
                                    myMarkers.add(Marker(
                      ...
                      return SingleChildScrollView(
                        child: Stack(
                          children: [
                                                  ...
                                                  children: [
                                                      ...
                                                      child: Row(
                                                        ...
                                                        children: [
                                                          ...
                                                          SizedBox(
                                                            width: media.width * 0.8,
                                                            child: Text(
                                                              languages[choosenLanguage]['text_background_permission'],
                                                              style: GoogleFonts.roboto(
                                                                  fontSize: media.width * fourteen,
                                                                  fontWeight: FontWeight.w600),
                                                            ),
                                                    ...
                                                    Container(
                                                        ...
                                                        child: Button(
                                                            ...
                                                            text: languages[choosenLanguage]['text_allow']))
                                                  ],
                                                ),
                                              )
                                            : (state == '3')
                                                    ...
                                                    children: [
                                                              ...
                                                              ? Positioned(
                                                                  ...
                                                                  child: AnimatedContainer(
                                                                    ...
                                                                    decoration: BoxDecoration(
                                                                      ...
                                                                      boxShadow: const [
                                                                        BoxShadow(
                                                                          color: ui.Color.fromARGB(255, 8, 38, 172),
                                                      ...
                                                      Positioned(
                                                          ...
                                                          child: Column(
                                                            ...
                                                            children: [
                                                              (driverReq.isNotEmpty && driverReq['is_trip_start'] == 1)
                                                                  ? InkWell(
                                                                      onTap: () async {
                                                                        setState(() {
                                                                          showSos = true;
                                                              ...
                                                              (driverReq.isNotEmpty && driverReq['accepted_at'] != null)
                                                                  ? InkWell(
                                                                      onTap: () async {
                                                                                                            ...
                                                                                                            ? SizedBox(
                                                                                                                ...
                                                                                                                child:
                                                                                                                  ...
                                                                                                                  child:
                                                                                                                    ...
                                                                                                                    style:
                                                                                                                        GoogleFonts.roboto(fontSize: media.width * fourteen, color: textColor),
                                                                                                                  ),
                                                                                                                ),
                                                                                                              )
                                                                                                            : Container()
                                          ...
                                          Container(
                                            ...
                                            child: Column(
                                              children: [
                                                ...
                                                SizedBox(height: media.width * 0.05),
                                                Text(
                                                  languages[choosenLanguage]['text_enterdriverotp'],
                                                  style: GoogleFonts.roboto(
                                                    fontSize: media.width * twelve,
                                                ...
                                                (_errorOtp == true)
                                                    ? Text(
                                                        ...
                                                        style: GoogleFonts.roboto(
                                                            color: Colors.red, fontSize: media.width * twelve),
                                                      )
                                                    : Container(),
                                                ...
                                                Button(
                                                  ...
                                                  color: (driverOtp.length != 4) ? Colors.grey : buttonColor,
                                                  borcolor: (driverOtp.length != 4) ? Colors.grey : buttonColor,
                                                )
                                              ],
                                            ),
                                          ),
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        Container(
                                          ...
                                          width: media.width * 0.9,
                                          decoration:
                                              BoxDecoration(color: page, borderRadius: BorderRadius.circular(12)),
                                          child: Column(children: [
                                            Container(
                                              height: media.width * 0.18,
                                              width: media.width * 0.18,
                                              decoration:
                                                  const BoxDecoration(shape: BoxShape.circle, color: Color(0xffFEF2F2)),
                                              alignment: Alignment.center,
                                              child: Container(
                                            ...
                                            (_cancelReason == 'others')
                                                ? Container(
                                                    margin: EdgeInsets.fromLTRB(
                                                        0, media.width * 0.025, 0, media.width * 0.025),
                                                    padding: EdgeInsets.all(media.width * 0.05),
                                                    width: media.width * 0.9,
                                                    decoration: BoxDecoration(
                                            ...
                                            (_cancellingError != '')
                                                ? Container(
                                                    ...
                                                    child: Text(_cancellingError,
                                                        style: GoogleFonts.roboto(
                                                            fontSize: media.width * twelve, color: Colors.red)))
                                                : Container(),
                                            Row(
                                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                              children: [
                                                    ...
                                                    onTap: () async {
                                                      ...
                                                      if (_cancelReason != '') {
                                                        if (_cancelReason == 'others') {
                                                          if (cancelReasonText != '' && cancelReasonText.isNotEmpty) {
                                                            _cancellingError = '';
                                                            await cancelRequestDriver(cancelReasonText);
                                                            setState(() {
                                                              cancelRequest = false;
                                                            });
                                                          } else {
                                                          ...
                                                          await cancelRequestDriver(_cancelReason);
                                                          setState(() {
                                                            cancelRequest = false;
                                                          });
                                                        }
                            ...
                            //loader
                            (state == '') ? const Positioned(top: 0, child: Loading()) : Container(),
                                            ...
                                            child: Column(
                                              children: [
                                                ...
                                                Row(
                                                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                                  children: [
                                                    Text(languages[choosenLanguage]['text_waiting_time_2'],
                                                        style: GoogleFonts.roboto(
                                                ...
                                                Row(
                                                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                                  children: [
                                                    Text(languages[choosenLanguage]['text_waiting_time_3'],
                                                        style: GoogleFonts.roboto(
                                                            fontSize: media.width * fourteen, color: textColor)),
                                                    Text(
                                                          ...
                                                          children: sosData
                                                              ...
                                                              .map((i, value) {
                                                                return MapEntry(
                                                                    ...
                                                                    InkWell(
                                                                      onTap: () {
                                                                        makingPhoneCall(sosData[i]['number']
                                                                            .toString()
                                                                            .replaceAll(' ', ''));
                                                                      },
                                                                      child: Container(
                                    ...
                                    children: [
                                        ...
                                        child: Row(
                                          children: [
                                            ...
                                            Expanded(
                                                child: Container(
                                              padding:
                                                  EdgeInsets.only(left: media.width * 0.02, right: media.width * 0.02),
                                              child: Column(
                                                mainAxisAlignment: MainAxisAlignment.spaceAround,
                                                crossAxisAlignment: CrossAxisAlignment.start,
                                                children: [
                                                  Text(
                                                    ...
                                                    style: GoogleFonts.roboto(
                                                        fontSize: media.width * twelve, fontWeight: FontWeight.bold),
                                                  ),
                                                  (driverReq.isNotEmpty && driverReq['pick_address'] != null)
                                                      ? Text(
                                      ...
                                      Container(
                                        decoration: const BoxDecoration(
                                            shape: BoxShape.circle,
                                            image: DecorationImage(
                                                image: AssetImage('assets/images/userloc.png'), fit: BoxFit.contain)),
                                        height: media.width * 0.05,
                                        width: media.width * 0.05,
                                      )
                                    ],
  ...
  animateCar(
    ...
    if (driverReq.isEmpty || driverReq['is_trip_start'] == 1) {
      controller.getVisibleRegion().then((value) {
        if (value.contains(myMarkers.firstWhere((element) => element.markerId == MarkerId(markerid)).position)) {
        } else {
...
class OwnerCarImagecontainer extends StatelessWidget {
  ...
  const OwnerCarImagecontainer(
      {Key? key, required this.imgurl, required this.text, required this.ontap, required this.color})
      : super(key: key);
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Container(
              decoration: BoxDecoration(
                  shape: BoxShape.circle, image: DecorationImage(image: AssetImage(imgurl), fit: BoxFit.contain)),
              height: media.width * 0.07,
              width: media.width * 0.15,
            ),
            Container(
